...
impl pgn_reader::Visitor for GameVisitor {
    ...
    fn header(&mut self, key: &[u8], value: pgn_reader::RawHeader<'_>) {
        let inner = if let Some(val) =
            self.try_get_inner() { val } else { return; };
    ...
    fn san(&mut self, san_plus: shakmaty::san::SanPlus) {
        ...
        let cur_node = if let Some(val) =
            inner.variation_stack.last_mut() { val } else { return; };
    ...
    fn begin_variation(&mut self) -> pgn_reader::Skip {
        let inner = if let Some(val) =
            self.try_get_inner() { val } else { return pgn_reader::Skip(true); };

        let cur_node = if let Some(val) =
            inner.variation_stack.last_mut() { val } else { return pgn_reader::Skip(true); };
    ...
    fn end_game(&mut self) -> Self::Result {
        let inner = if let Some(val) =
            self.try_get_inner() { val } else { return GameTree::default(); };
